...
class My2048:
    ...
    def move(self, direction, no_spawn=False):
        ...
        self.checkerboard = np.rot90(self.checkerboard, k=rotation_mapping[direction], axes=(0, 1))

        # Take move.
        is_moved = self._move_left()

        # Rotate back.
        self.checkerboard = np.rot90(self.checkerboard, k=4-rotation_mapping[direction], axes=(0, 1))

        # Spawn new block.
        if is_moved and not no_spawn:
            self._spawn_new()
